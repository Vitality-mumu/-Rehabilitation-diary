...
Page({
  ...
  async addHeartRate() {
    try {
      let res = await http.addHeartRate(this.data.id, this.data.heartrate,this.data.date);
      this.setData({
        heartrateShow: false,
        heartrate: null,
      });
  ...
  async addTemperature() {
    try {
      let res = await http.AddTemperature(this.data.id, this.data.temperature,this.data.date);
      this.setData({
        temperatureShow: false,
        temperature: "",
      });
  ...
  async addWeight() {
    try {
      let res = await http.addWeight(this.data.id, this.data.weight,this.data.date);
      this.setData({
        weightShow: false,
        weight: null
      });
  ...
  async addMenses() {
    try {
      let res = await http.addMenses(this.data.id, this.data.mensesStatus,this.data.date);
      console.log(res);
      if (res) {
        if (this.data.mensesStatus == 1) {
          this.setData({
